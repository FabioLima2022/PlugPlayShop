    ...
    if (!check_csrf($_POST['csrf'] ?? null)) {
        ...
        if ($email !== '' && $pass !== '') {
            ...
            if ($attempt && !empty($attempt['locked_until'])) {
                ...
                if ($lockedUntil > $now) { $error = 'Muitas tentativas. Tente novamente após ' . $lockedUntil->format('H:i'); }
            }

            if ($error === '') {
                $stmt = $db->prepare('SELECT id, username, password_hash, role FROM users WHERE (username = ? OR email = ?) AND role = "affiliate" LIMIT 1');
                ...
                if ($row && password_verify($pass, $row['password_hash'])) {
                    ...
                    if ($attempt) {
                        ...
                        $lockedUntilVal = null;
                        if ($count >= $maxAttempts) { $locked = (clone $now)->modify('+' . $lockMinutes . ' minutes')->format('Y-m-d H:i:s'); $lockedUntilVal = $locked; }
                        ...
                        $stmt->bind_param('sss', $email, $ip, $lockedUntilVal);
                        $stmt->execute();
                        $stmt->close();
                    }
...
<body>
  <header class="site-header">
    <div class="container header-inner">
      ...
      <nav class="actions"><a class="btn" href="index.php">Voltar à loja</a><a class="btn" href="affiliate_register.php">Cadastre-se</a><a class="btn" href="affiliate_forgot.php">Recuperar senha</a></nav>
    </div>
  </header>
